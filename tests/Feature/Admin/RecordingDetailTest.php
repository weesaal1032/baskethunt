...
class RecordingDetailTest extends TestCase
{
    ...
    public function test_user_can_view_recording_detail_page(): void
    {
        ...
        $this->assertTrue(
            Audit::query()->where('action', 'recording.view')->exists(),
            'Recording view should be audited.'
        );
    }
    ...
    public function test_signed_audio_route_streams_local_recording(): void
    {
        ...
        $this->assertTrue(
            Audit::query()->where('action', 'recording.audio.stream')->where('subject_id', $recording->id)->exists(),
            'Audio stream should be audited.'
        );
    }
    ...
    public function test_signed_audio_route_redirects_for_s3_recording(): void
    {
        ...
        $this->assertTrue(
            Audit::query()->where('action', 'recording.audio.stream')->where('subject_id', $recording->id)->exists(),
            'S3 audio redirect should be audited.'
        );
    }

    public function test_guest_cannot_access_recording_audio(): void
    {
        $call = $this->createCall();
        $recording = Recording::query()->create([
            'call_id' => $call->id,
        ...
        $signedUrl = URL::temporarySignedRoute('admin.recordings.audio', now()->addMinute(), [
            'recording' => $recording->id,
        ]);
        ...
        $this->assertFalse(
            Audit::query()->where('action', 'recording.audio.stream')->exists(),
            'Guests should not emit audit entries.'
        );
    }
    ...
    public function test_unsigned_request_is_rejected(): void
    {
        ...
        $response = $this->actingAs($viewer)->get(route('admin.recordings.audio', $recording));

        $response->assertStatus(403);
        $this->assertFalse(
            Audit::query()->where('action', 'recording.audio.stream')->exists(),
            'Unsigned attempts should not be audited as successful streams.'
        );
    }

    public function test_inactive_user_cannot_view_recording_detail(): void
    {
        $viewer = User::factory()->create(['role' => 'qa', 'status' => 'suspended']);
        $call = $this->createCall($viewer->id);
        $recording = Recording::query()->create([
            'call_id' => $call->id,
            'remote_url' => 'https://example.test/audio.mp3',
            'local_path' => 'recordings/suspended.mp3',
            'storage_backend' => 'local',
            'format' => 'mp3',
            'status' => 'ready',
        ]);
        ...
        $this->assertFalse(
            Audit::query()->where('action', 'recording.view')->exists(),
            'Unauthorized access should not be audited as a view.'
        );
    }
